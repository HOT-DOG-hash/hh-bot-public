...
from utils.states import CL_ASK_TITLE, CL_SAVE_BODY, CL_VIEW, COVER_LETTER_MENU
...
async def show_cover_letters(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for i, letter in enumerate(cover_letters):
        keyboard.append(
            [InlineKeyboardButton(f"📄 {letter['title']}", callback_data=f"cl_view_{i}")]
        )
...
async def ask_new_cover_letter_title(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(context.user_data.get("cover_letters", [])) >= 5:
        keyboard = [
            [InlineKeyboardButton("🔙 Вернуться в список писем", callback_data="cl_back_to_list")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.callback_query.message.edit_text(
            texts.CL_LIMIT_REACHED, reply_markup=reply_markup
        )
        return COVER_LETTER_MENU
