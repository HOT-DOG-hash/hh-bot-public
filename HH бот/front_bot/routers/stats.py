...
async def show_stats_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not resumes:
        ...
        if update.callback_query:
            ...
            await update.callback_query.message.edit_text(
                texts.STATS_NO_RESUMES, reply_markup=reply_markup
            )
        else:
            await update.message.reply_text(texts.STATS_NO_RESUMES, reply_markup=reply_markup)
    ...
    for resume in resumes:
        keyboard.append(
            [InlineKeyboardButton(resume["name"], callback_data=f"stats_resume_{resume['id']}")]
        )
    ...
    if update.callback_query:
        await update.callback_query.answer()
        await update.callback_query.message.edit_text(
            texts.STATS_SELECT_RESUME, reply_markup=reply_markup
        )
    else:
        await update.message.reply_text(texts.STATS_SELECT_RESUME, reply_markup=reply_markup)
...
def get_stats_conv_handler():
    ...
    return ConversationHandler(
        entry_points=[CallbackQueryHandler(show_stats_menu, pattern="^stats$")],
        states={
